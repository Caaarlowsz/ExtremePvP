		...
		if (cmd.getName().equalsIgnoreCase("tp")) {
			...
			if (args.length == 0) {
				...
				if ((p.hasPermission("tk.tp")) || (p.isOp())) {
					...
					p.teleport(tpLoc);
					p.sendMessage(ChatColor.GRAY + "§6Teleportado para: " + targetPlayer.getDisplayName()
							+ ChatColor.GRAY + ".");
				} else {
				...
				if ((p.hasPermission("TK.tp")) || (p.isOp())) {
					...
					if ((targetPlayer == null) || (targetPlayer2 == null)) {
						...
						p.sendMessage(ChatColor.GRAY + "§eExtreme§fKits » §6Teleportado para: "
								+ targetPlayer.getDisplayName() + ChatColor.GRAY + ".");
					}
				} else {
					p.sendMessage("§eExtreme§fKits » §cComando Incorreto");
				}
