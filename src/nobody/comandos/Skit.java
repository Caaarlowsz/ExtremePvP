		...
		if ((Label.equalsIgnoreCase("skit")) && ((Sender instanceof Player))) {
			...
			if (Args[0].equalsIgnoreCase("aplicar")) {
				...
				if (isInt(Args[2])) {
					...
					Bukkit.getServer().broadcastMessage("   §eExtreme§bKits §8> O Kit §a" + nome
							+ " §afoi Aplicado para Players em um Raio de §a" + n + " §eBlocos");
					Bukkit.getServer().broadcastMessage("");
					p.sendMessage(
							"§aVoce aplicou o Kit §e" + nome + " §aPara Players em um Raio de §a" + n + " §eBlocos");
					return true;
				}
			}
		}
