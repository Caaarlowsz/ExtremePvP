		...
		new BukkitRunnable() {
			...
			public void run() {
				...
				if (refillTest.this.hasRefiled(p)) {
					p.setExp(0.0F);
					p.sendMessage("§eExtreme§fKits »  §7Você terminou seu refil , ele foi feito em §c"
							+ StringTimer.timerChecker(((Integer) KitAPI.refillTestTime.get(p.getName())).intValue()));
				...
				if (this.seconds >= 60) {
					p.sendMessage(
							"§eExtreme§fKits »  §cSe passou 1 minuto e você não terminou o refil , teste cancelado");
					cancel();
				}
			}
