...
/*     */ {
		...
		/* 31 */ (event.getAction() == org.bukkit.event.block.Action.RIGHT_CLICK_BLOCK)) {
			...
			/*     */ {
				...
				/*     */ {
					...
					/* 67 */ Bukkit.getServer().getScheduler().scheduleSyncDelayedTask(Main.getInstance(),
							new Runnable()
							/*     */ {
								/*     */ public void run()
								/*     */ {
	...
	/*     */ public void veneno(PlayerMoveEvent event) {
		...
		/* 186 */ if (KitAPI.getkit(p) == "JellyFish") {
			...
			/* 188 */ } else if ((b.getType() == Material.STATIONARY_WATER)
					&& (!p.getInventory().contains(Material.CLAY_BALL)) && (this.naoescorrer.contains(b))) {
 ...
 * plugins\MasterKits.jar!\Kits\JellyFish.class Java compiler version: 7 (51.0)
