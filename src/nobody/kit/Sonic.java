...
/*     */ {
	...
	/*     */ {
		...
		/* 38 */ if ((e.getPlayer().getItemInHand().getType() == Material.LAPIS_BLOCK)
				&& (KitAPI.getkit(p) == "Sonic")) {
			/* 39 */ if ((e.getAction() == Action.LEFT_CLICK_AIR) || (e.getAction() == Action.LEFT_CLICK_BLOCK)
			...
			/*     */
			/* 89 */ org.bukkit.Bukkit.getServer().getScheduler().scheduleSyncDelayedTask(Main.getInstance(),
					new Runnable() {
						/*     */ public void run() {
					...
					50L);
			/*     */
			/* 99 */ org.bukkit.Bukkit.getServer().getScheduler().scheduleSyncDelayedTask(Main.getInstance(),
					new Runnable() {
						/*     */ public void run() {
	...
	/*     */ public void onPlayerDamageSponge(EntityDamageEvent e) {
		...
		/* 115 */ if ((e.getCause() == org.bukkit.event.entity.EntityDamageEvent.DamageCause.FALL)
				&& (this.fall.contains(p.getName()))) {
 ...
 * plugins\MasterKits.jar!\Kits\Sonic.class Java compiler version: 7 (51.0)
