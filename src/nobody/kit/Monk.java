			...
			/* 22 */ if ((KitAPI.getkit(p) == "Monk") && (p.getItemInHand().getType() == Material.BLAZE_ROD)) {
				...
				/* 24 */ if (!KitAPI.KitDelay.containsKey(p.getName())) {
					...
					/*    */ {
						/*    */ public void run()
						/*    */ {
							/* 37 */ if ((KitAPI.KitDelay.containsKey(p.getName())) && (KitAPI.getkit(p) == "Monk")
						...
						/* 43 */ }, 199L);
					/*    */ } else {
					/* 45 */ p.sendMessage("§7[§6§l!§7] §cMonk em cooldown aguarde §f"
							+ KitAPI.KitDelay.get(p.getName()) + " §csegundos para usar");
					/*    */ }
				/*    */ }
			/*    */ }
		/*    */ }
 ...
 * plugins\MasterKits.jar!\Kits\Monk.class Java compiler version: 7 (51.0)
